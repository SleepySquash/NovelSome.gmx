...
if (global.selChatTab!=tab||(!global.chat_open && !alphaOn)) {cp=1; if (global.loggedIn) {
        if (string_copy(argument0,1,string_length("<"+string(global.username)+">"))=="<"+string(global.username)+">") {cp=0;}}
    if (cp) {smNew=1; if (!notNot) {global.smNew=1; if (global.soundIs) {
...
if (string_width(argument0) > chat_x2)
{
    ...
    while (i <= length)
    {
        ...
        if (string_width(working_string) > chat_x2)
        {
            ...
            chat_pretitle[j] = "";
            chat_posttitle[j] = "";
            chat_add(string_copy(argument0,i-k,string_length(argument0)-i+1+k));
            exit;
        }    
        i += 1;
    }
