...
*/
{
    ...
    bt = yy - h;
    bb = yy + h;
    if (b) {/*draw_sprite_ext(sprGUI_buttonback,-1,xx-w,yy-h,w,h,0,c_white,1);*/
        draw_set_alpha(prev_alpha/2.5);/*0.4*/ draw_rectangle_colour(xx-w,yy-h,xx+w,yy+h,c_black,c_black,c_black,c_black,false);}
    in = (mouse_x>bl && mouse_x<br && mouse_y>bt && mouse_y<bb && !global.mouseUnderNot);
    if (in && global.buttonClickble) {
        draw_set_colour(col3);
        draw_set_alpha(prev_alpha/1.25); //0.2
        draw_rectangle_colour(bl,bt,br,bb,col3,col3,col3,col3,0);
        draw_set_colour(col2);
        draw_set_alpha(prev_alpha); //1
    }else{
        draw_set_colour(col1);
        draw_set_alpha(prev_alpha/1.42);}
    draw_rectangle_colour(bl,bt,br,bb,draw_get_colour(),draw_get_colour(),draw_get_colour(),draw_get_colour(),1);
